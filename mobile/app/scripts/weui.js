...
var weui = {
    ...
    confirm: function(msg, title, callback) {
        var confirmHtml = '<div class="weui_dialog_confirm">' + '<div class="weui_mask"></div>' + '<div class="weui_dialog">' + '<div class="weui_dialog_hd"><strong class="weui_dialog_title"></strong></div>' + '<div class="weui_dialog_bd"></div>' + '<div class="weui_dialog_ft">' + '<a href="javascript:;" class="weui_btn_dialog default">取消</a>' + '<a href="javascript:;" class="weui_btn_dialog primary" style="background-color:#fa952f;">确定</a>' + '</div>' + '</div>' + '</div>';
    ...
    Loading: {
        show: function(msg) {
            ...
            weui_loading.find(".weui_toast").css("left", (w - width) / 2+50);
        },
        hide: function() {
            if (!weui.destroyFlag)
        ...
        msg: function(msg, time, cls, icon) {
            var successHtml = '<div class="' + cls + '" style="display: none;">' + '<div class="weui_mask_transparent"></div>' + '<div class="weui_toast">' + '<i class="' + icon + '"></i>' + '<p class="weui_toast_content"></p>' + '</div>' + '</div>';
            if ($("." + cls).length < 1) {
            ...
            var w = $(document).width();
            weui_success.find(".weui_toast").css("left", (w- width)/2+50);
            setTimeout(function() {
                weui_success.hide();
            }, t);
        },
        success: function(msg, time) {
             this.msg(msg, time, "weui_success_toast", "");
